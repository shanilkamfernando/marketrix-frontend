...
function VideoModalFocusMode({
  ...
  const sendQuestion = async (currentSpeechText) => {
    try {
      ...
      console.log("WORKED__________", req);
  ...
  const welcomeTheUser = () => {
    ...
    audioPlayerWelcome.current.addEventListener("playing", () => {
      // clearTimeout(timeOut);
      talkingState();
    });
  ...
  const continuePlay = () => {
    console.log("continuePlay ________________________", audioArray.length);
    console.log(
      "continuePlay  audioArray ________________________",
      audioArray
    );
    if (audioArray.length > 0) {
  ...
  useEffect(() => {
    console.log("Audio Array Changed...", audioArray);
    //continuePlay();
  }, [audioArray]);
